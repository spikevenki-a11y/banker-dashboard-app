...
import { ArrowLeft, Plus, Search, Eye, FileText, TrendingUp, Calendar, RefreshCw, Loader2, AlertCircle, Banknote, Info } from "lucide-react"
...
const actionContext: Record<string, { title: string; description: string; presetStatus?: string }> = {
  transactions: {
    title: "Select an account for Transactions",
    description: "Choose a deposit account below to view or post transactions against it.",
  },
  closure: {
    title: "Select an account to close",
    description: "Choose an active or matured deposit account below, then use Closure from its Actions menu.",
  },
  renew: {
    title: "Select an account to renew",
...
function FixedDepositsAccountsContent() {
  ...
  const [statusFilter, setStatusFilter] = useState(context?.presetStatus || "all")
      ...
      <div className="">
        <div className="">
          <main className="flex-1 overflow-y-auto bg-background p-6">
            ...
            {context && (
              <div className="mb-6 flex items-center gap-2 rounded-lg border border-teal-200 bg-teal-50 px-4 py-3 text-sm dark:border-teal-800 dark:bg-teal-950">
                <Info className="h-4 w-4 shrink-0 text-teal-600" />
                <div>
                  <p className="font-medium text-teal-800 dark:text-teal-300">{context.title}</p>
                  <p className="text-teal-700 dark:text-teal-400">{context.description}</p>
                </div>
              </div>
            )}
...
export default function FixedDepositsAccountsPage() {
  return (
    <Suspense
      fallback={
        <DashboardWrapper>
