...
import { ArrowLeft, Printer, Loader2, Search, DollarSign, FileDown } from "lucide-react"
...
const TABS = [
  { value: "register",     label: "Share Register" },
  { value: "transactions", label: "Share Transactions" },
]

function SharesReportContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const [activeTab, setActiveTab] = useState(searchParams.get("type") || "register")
  ...
  async function fetchReport(tab = activeTab) {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/shares?${p}`, { credentials: "include" })
  ...
  function handlePrint() {
    ...
    const title = TABS.find(t => t.value === activeTab)?.label || "Share Report"
    ...
    <DashboardWrapper>
      <div className="space-y-6 p-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Share Reports</h1>
            <p className="text-muted-foreground">View and print share register and transaction reports</p>
          </div>
        </div>
            ...
            <Card className="mt-4">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm text-muted-foreground flex items-center gap-2">
                  ...
                  {summary?.total_share_amount !== undefined && (
                    <span className="ml-4 font-normal">
                      · Total Share Amount: <strong>{fmt(summary.total_share_amount)}</strong>
                      {" "}({summary.active_members} active members)
                    </span>
                  )}
                  {summary?.total_credit !== undefined && (
                    <span className="ml-4 font-normal">
                      · Credits: <strong>{fmt(summary.total_credit)}</strong>
                      {" "}· Debits: <strong>{fmt(summary.total_debit)}</strong>
                    </span>
                  )}
                </CardTitle>
              </CardHeader>
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <TableHead className="text-xs">Share Opened</TableHead>
                            <TableHead className="text-xs">Closing Date</TableHead>
                            <TableHead className="text-xs">Member Status</TableHead>
                            <TableHead className="text-xs">Share Status</TableHead>
                            <TableHead className="text-right text-xs">Share Amount</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {rows.map((r, i) => (
                            <TableRow key={i}>
                              ...
                              <TableCell className="text-xs">{fmtDate(r.share_opened_date)}</TableCell>
                              <TableCell className="text-xs">{fmtDate(r.closing_date)}</TableCell>
                              <TableCell>
                                <Badge variant="outline" className={`text-[10px] ${r.member_status === "ACTIVE" ? "border-teal-300 text-teal-700" : "border-red-300 text-red-700"}`}>
                                  {r.member_status}
                                </Badge>
                              </TableCell>
                              <TableCell>
                                <Badge variant="outline" className={`text-[10px] ${r.status === "ACTIVE" ? "border-teal-300 text-teal-700" : "border-slate-300 text-slate-700"}`}>
                                  {r.status}
                                </Badge>
                              </TableCell>
                              <TableCell className="text-right text-xs font-mono font-medium">{fmt(r.share_amount)}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                        {rows.length > 0 && (
                          <TableFooter>
                            <TableRow>
                              <TableCell colSpan={8} className="text-xs font-bold">Total (Active)</TableCell>
                              <TableCell className="text-right text-xs font-bold font-mono">{fmt(summary?.total_share_amount)}</TableCell>
                            </TableRow>
                          </TableFooter>
                        )}
                      </Table>
                    ...
                    {activeTab === "transactions" && (
                      <Table>
                        ...
                        <TableBody>
                          {rows.map((r, i) => (
                            <TableRow key={i}>
                              ...
                              <TableCell>
                                <Badge variant="outline" className={`text-[10px] ${r.status === "APPROVED" ? "border-teal-300 text-teal-700" : r.status === "REJECTED" ? "border-red-300 text-red-700" : "border-amber-300 text-amber-700"}`}>
                                  {r.status}
                                </Badge>
                              </TableCell>
                              <TableCell className="text-xs max-w-[150px] truncate">{r.narration || "—"}</TableCell>
                        ...
                        {rows.length > 0 && (
                          <TableFooter>
                            <TableRow>
                              <TableCell colSpan={7} className="text-xs font-bold">Total</TableCell>
