...
export default function AccountOpeningForm({ onAccountCreated }: { onAccountCreated: () => void }) {
  ...
  useEffect(() => {
    const fetchGLAccounts = async () => {
      try {
        const response = await fetch(`/api/expenses/ledger-accounts`)
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Open New Expense Account</CardTitle>
        <CardDescription>
          Create a new expense account linked to a Chart of Accounts ledger
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="glAccountCode">Ledger Account (Expense Head) *</Label>
