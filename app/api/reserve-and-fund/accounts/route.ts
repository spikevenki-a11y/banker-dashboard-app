...
export async function GET() {
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM reserve_and_fund_master
        WHERE branch_id = $1
    ...
    console.error("Reserve & Fund GET Accounts error:", error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { rows: [countRow] } = await pool.query(
      `SELECT COUNT(*) AS count FROM reserve_and_fund_master WHERE branch_id = $1`,
      [branchId]
    )
    ...
    const result = await pool.query(
      `INSERT INTO reserve_and_fund_master (
    ...
    console.error("Reserve & Fund POST Account error:", error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
}
