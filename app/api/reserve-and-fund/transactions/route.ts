  ...
  try {
    ...
    const { rows: transactions } = await pool.query(
      `SELECT
       ...
       FROM reserve_and_fund_transactions rft
       LEFT JOIN gl_batches gb ON gb.branch_id = rft.branch_id AND gb.batch_id = rft.reference_no::BIGINT
       WHERE rft.account_number = $1 AND rft.branch_id = $2
       ORDER BY rft.transaction_date DESC, rft.created_at DESC`,
      [accountNumber, branchId]
    )

    return NextResponse.json(transactions)
  } catch (error: any) {
    console.error("Reserve & Fund GET Transactions error:", error)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const processedEntries = entries.map((e: any) => ({
      ...
      description: e.description || `Reserve & Fund ${transaction_type}`,
    }))
    ...
    /* --- CASH GL LINE --- */
    // Reserve & Fund are equity-like: CREDIT increases (DR Cash, CR Fund GL), DEBIT decreases (CR Cash, DR Fund GL)
    if (transaction_type === "CREDIT") {
      await client.query(
        ...
        [session.branch, batchId, businessDate, cashGl, totalAmount, voucherNo,
          "Reserve & Fund - Receipt", session.userId]
      )
    } else {
      await client.query(
        `INSERT INTO gl_batch_lines (branch_id, batch_id, business_date, accountcode, ref_account_id,
        ...
        [session.branch, batchId, businessDate, cashGl, totalAmount, voucherNo,
          "Reserve & Fund - Payment", session.userId]
      )
    }
    ...
    for (const entry of processedEntries) {
      const { rows: accounts } = await client.query(
        `SELECT rfm.*, coa.accountcode AS gl_accountcode
         FROM reserve_and_fund_master rfm
         LEFT JOIN chart_of_accounts coa ON coa.accountcode = rfm.parent_account_number::BIGINT
         WHERE rfm.account_number = $1 AND rfm.branch_id = $2
         FOR UPDATE OF rfm`,
      ...
      await client.query(
        `INSERT INTO reserve_and_fund_transactions (
      ...
      await client.query(
        `UPDATE reserve_and_fund_master SET ledger_balance = $1
    ...
    return NextResponse.json({
      ...
      message: `${processedEntries.length} reserve & fund transaction(s) recorded. Batch: ${batchId}, Voucher: ${voucherNo}. Total: ₹${totalAmount.toLocaleString("en-IN")}`,
    }, { status: 201 })
  } catch (error: any) {
    await client.query("ROLLBACK")
    console.error("Reserve & Fund POST Transaction error:", error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  } finally {
    client.release()
  }
