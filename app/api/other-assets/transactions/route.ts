  ...
  try {
    ...
    const { rows: transactions } = await pool.query(
      `SELECT
       ...
       FROM other_assets_transactions oat
       LEFT JOIN gl_batches gb ON gb.branch_id = oat.branch_id AND gb.batch_id = oat.reference_no::BIGINT
       WHERE oat.account_number = $1 AND oat.branch_id = $2
       ORDER BY oat.transaction_date DESC, oat.created_at DESC`,
      [accountNumber, branchId]
    )

    return NextResponse.json(transactions)
  } catch (error: any) {
    console.error("Other Assets GET Transactions error:", error)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const processedEntries = entries.map((e: any) => ({
      ...
      description: e.description || `Other Asset ${transaction_type}`,
    }))
    ...
    if (transaction_type === "DEBIT") {
      // DEBIT: asset increases — DR Asset GL, CR Cash
      await client.query(
        ...
        [session.branch, batchId, businessDate, cashGl, totalAmount, voucherNo,
          "Other Asset - Payment", session.userId]
      )
    } else {
      // CREDIT: asset decreases — CR Asset GL, DR Cash
      await client.query(
        `INSERT INTO gl_batch_lines (
           branch_id, batch_id, business_date,
           accountcode, ref_account_id,
        ...
        [session.branch, batchId, businessDate, cashGl, totalAmount, voucherNo,
          "Other Asset - Receipt", session.userId]
      )
    }
    ...
    for (const entry of processedEntries) {
      const { rows: accounts } = await client.query(
        `SELECT oam.*, coa.accountcode AS gl_accountcode
         FROM other_assets_master oam
         LEFT JOIN chart_of_accounts coa ON coa.accountcode = oam.parent_account_number::BIGINT
         WHERE oam.account_number = $1 AND oam.branch_id = $2
         FOR UPDATE OF oam`,
      ...
      await client.query(
        `INSERT INTO other_assets_transactions (
    ...
    return NextResponse.json({
      ...
      message: `${processedEntries.length} other asset transaction(s) recorded. Batch: ${batchId}, Voucher: ${voucherNo}. Total: ₹${totalAmount.toLocaleString("en-IN")}`,
    }, { status: 201 })
  } catch (error: any) {
    await client.query("ROLLBACK")
    console.error("Other Assets POST Transaction error:", error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  } finally {
    client.release()
  }
