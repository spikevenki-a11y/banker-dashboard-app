...
export async function GET() {
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM other_assets_master
        WHERE branch_id = $1
    ...
    console.error("Other Assets GET Accounts error:", error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { rows: [countRow] } = await pool.query(
      `SELECT COUNT(*) AS count FROM other_assets_master WHERE branch_id = $1`,
      [branchId]
    )
    ...
    const result = await pool.query(
      `INSERT INTO other_assets_master (
    ...
    console.error("Other Assets POST Account error:", error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
}
