...
export async function GET() {
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM other_liabilities_master
        WHERE branch_id = $1
    ...
    console.error("Other Liabilities GET Accounts error:", error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Sequential account number per branch
    const { rows: [countRow] } = await pool.query(
      `SELECT COUNT(*) AS count FROM other_liabilities_master WHERE branch_id = $1`,
      [branchId]
    )
    ...
    const result = await pool.query(
      `INSERT INTO other_liabilities_master (
    ...
    console.error("Other Liabilities POST Account error:", error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
}
