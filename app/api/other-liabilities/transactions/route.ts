  ...
  try {
    ...
    const { rows: transactions } = await pool.query(
      `SELECT
       ...
       FROM other_liabilities_transactions olt
       LEFT JOIN gl_batches gb ON gb.branch_id = olt.branch_id AND gb.batch_id = olt.reference_no::BIGINT
       WHERE olt.account_number = $1 AND olt.branch_id = $2
       ORDER BY olt.transaction_date DESC, olt.created_at DESC`,
      [accountNumber, branchId]
    )

    return NextResponse.json(transactions)
  } catch (error: any) {
    console.error("Other Liabilities GET Transactions error:", error)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const processedEntries = entries.map((e: any) => ({
      ...
      description: e.description || `Other Liability ${transaction_type}`,
    }))
    ...
    if (transaction_type === "CREDIT") {
      // CREDIT: liability increases — DR Cash, CR Liability GL
      await client.query(
        ...
        [session.branch, batchId, businessDate, cashGl, totalAmount, voucherNo,
          "Other Liability - Receipt", session.userId]
      )
    } else {
      // DEBIT: liability decreases — CR Cash, DR Liability GL
      await client.query(
        `INSERT INTO gl_batch_lines (
           branch_id, batch_id, business_date,
           accountcode, ref_account_id,
        ...
        [session.branch, batchId, businessDate, cashGl, totalAmount, voucherNo,
          "Other Liability - Payment", session.userId]
      )
    }
    ...
    for (const entry of processedEntries) {
      const { rows: accounts } = await client.query(
        `SELECT olm.*, coa.accountcode AS gl_accountcode
         FROM other_liabilities_master olm
         LEFT JOIN chart_of_accounts coa ON coa.accountcode = olm.parent_account_number::BIGINT
         WHERE olm.account_number = $1 AND olm.branch_id = $2
         FOR UPDATE OF olm`,
      ...
      await client.query(
        `INSERT INTO other_liabilities_transactions (
      ...
      await client.query(
        `UPDATE other_liabilities_master
    ...
    return NextResponse.json({
      ...
      message: `${processedEntries.length} other liability transaction(s) recorded. Batch: ${batchId}, Voucher: ${voucherNo}. Total: ₹${totalAmount.toLocaleString("en-IN")}`,
    }, { status: 201 })
  } catch (error: any) {
    await client.query("ROLLBACK")
    console.error("Other Liabilities POST Transaction error:", error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  } finally {
    client.release()
  }
