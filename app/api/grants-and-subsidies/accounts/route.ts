...
export async function GET() {
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM grands_and_subsidies_master
        WHERE branch_id = $1
    ...
    console.error("Grants & Subsidies GET Accounts error:", error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { rows: [countRow] } = await pool.query(
      `SELECT COUNT(*) AS count FROM grands_and_subsidies_master WHERE branch_id = $1`,
      [branchId]
    )
    ...
    const result = await pool.query(
      `INSERT INTO grands_and_subsidies_master (
    ...
    console.error("Grants & Subsidies POST Account error:", error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
}
