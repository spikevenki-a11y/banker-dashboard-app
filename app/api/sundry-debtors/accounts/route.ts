...
export async function GET() {
  ...
  try {
    ...
    // Fetch all sundry debtor accounts
    ...
    console.error("Error fetching sundry debtor accounts:", error)
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { rows: [seq] } = await pool.query(
      ...
      WHERE branch_id = $1 AND accounttype = 62
      RETURNING nextvalue
      `,
      [branchId]
    )
    ...
    if (seq) {
      ...
      await pool.query(
        `INSERT INTO nextnumber (branch_id, accounttype, nextvalue) VALUES ($1, 62, 1)
    ...
    console.error("Error creating sundry debtor account:", error)
    return NextResponse.json(
