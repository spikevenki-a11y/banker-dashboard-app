  ...
  try {
    ...
    const { rows: transactions } = await pool.query(
      `SELECT 
       ...
       FROM sundry_creditor_transactions sct
       LEFT JOIN gl_batches gb ON gb.branch_id = sct.branch_id AND gb.batch_id = sct.gl_batch_id
       WHERE sct.account_number = $1 AND sct.branch_id = $2
       ORDER BY sct.transaction_date DESC, sct.created_at DESC`,
      [accountNumber, branchId]
    )

    return NextResponse.json(transactions)
  } catch (error: any) {
    console.error('Error fetching sundry creditor transactions:', error)
    return NextResponse.json(
      { error: error.message || 'Failed to fetch sundry creditor transactions' },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const processedEntries = entries.map((e: any) => ({
      ...
      description: e.description || `Sundry Creditor ${transaction_type}`
    }))
    ...
    if (voucher_type === "CASH") {
      if (transaction_type === "CREDIT") {
        // DR Cash (Asset increases) when we receive goods/services on credit
        await client.query(`
        ...
        `, [
          ...
          totalAmount,
          voucherNo,
          "Sundry Creditor - Credit",
          session.userId
        ])
      } else {
        // CR Cash (Asset decreases) when we pay creditors
        await client.query(`
          INSERT INTO gl_batch_lines (
            branch_id, batch_id, business_date,
            accountcode, ref_account_id,
    ...
    for (const entry of processedEntries) {
      ...
      /* ---------------- GL LINES FOR SUNDRY CREDITOR ACCOUNT ---------------- */
      if (transaction_type === "CREDIT") {
        // CR Sundry Creditor GL (Liability increases)
        await client.query(`
          INSERT INTO gl_batch_lines (
            branch_id, batch_id, business_date,
            accountcode, ref_account_id,
        ...
        // DR Sundry Creditor GL (Liability decreases)
        await client.query(`
          INSERT INTO gl_batch_lines (
            branch_id, batch_id, business_date,
            accountcode, ref_account_id,
      ...
      /* ---------------- SUNDRY CREDITOR TRANSACTION ---------------- */
      await client.query(`
        INSERT INTO sundry_creditor_transactions (
      ...
      await client.query(
        `UPDATE sundry_creditors 
         SET ledger_balance = $1, updated_at = NOW()
    ...
    return NextResponse.json({
      ...
      message: `${processedEntries.length} sundry creditor transaction(s) recorded. Batch: ${batchId}, Voucher: ${voucherNo}. Total: ₹${totalAmount.toLocaleString("en-IN")}`,
    }, { status: 201 })
  } catch (error: any) {
    await client.query("ROLLBACK")
    console.error('Error creating sundry creditor transaction:', error)
    return NextResponse.json(
      { error: error.message || 'Failed to create sundry creditor transaction' },
      { status: 500 }
    )
  } finally {
    client.release()
